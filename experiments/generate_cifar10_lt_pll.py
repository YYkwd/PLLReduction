...
- Class order is fixed as original labels 0..9 (0=head, 9=tail).
- Default n1: fast=1000, full=3000.
- Read standard keys `data` and `labels` first; if missing, print actual keys.
...
def main() -> None:
    ...
    parser.add_argument('--name', type=str, default=None,
                        help='Optional dataset name. Default: cifar10-lt-g{gamma}-r{r}-{mode}')
    parser.add_argument('--append-seed-to-name', action='store_true',
                        help='Append -s{seed} suffix to dataset name to avoid overwrite')
    ...
    partial_target = build_partial_target(y_lt, r=args.r, rng=rng, n_classes=10)

    dataset_name = args.name or f'cifar10-lt-g{args.gamma}-r{args.r}-{args.mode}'
    if args.append_seed_to_name:
        dataset_name = f'{dataset_name}-s{args.seed}'
    ...
    avg_cands = float(partial_target.sum(axis=0).mean())
    min_cands = int(partial_target.sum(axis=0).min())
    max_cands = int(partial_target.sum(axis=0).max())
    expected_cands = int(1 + args.r)
    if min_cands != expected_cands or max_cands != expected_cands:
        raise ValueError(
            'Candidate cardinality mismatch: '
            f'min={min_cands}, max={max_cands}, expected={expected_cands}'
        )
    ...
    print(f'[INFO] mode={args.mode} seed={args.seed} n1={n1} gamma={args.gamma} r={args.r}')
