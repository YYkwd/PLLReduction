"""Ablation on target_d impact for Mirflickr dataset (baseline vs sr+cb)."""
...
def main() -> None:
    ...
    for d in D_LIST:
        for variant_name, variant_params in VARIANTS.items():
            ...
            # Mirflickr is much larger; use fast budget for timely ablation.
            apply_fast_training_overrides(cfg)

            cfg['_run_meta'] = {
                'run_id': f"td_mirflickr_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{d}_{variant_name}",
                'script_name': 'target_d_impact_mirflickr',
                'mode': 'fast',
            }
