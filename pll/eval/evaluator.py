...
from pll.config import reduction_identity_hash
...
def _register_defaults():
    ...
    from pll.classifiers.pl_svm import PLSVMClassifier
    from pll.classifiers.sure import SUREClassifier
...
class Evaluator:
    ...
    def _reduced_mats_for_split(self, X, y, partial_target, train_idx, test_idx,
                                cfg):
        """Z-score, fit reducer, transform — one call per split for all classifiers."""
    ...
    def run_multi_classifier(self, dataset, configs: list[dict]) -> list[EvalResult]:
        ...
        All ``configs`` must be identical except ``classifier`` (name/params). Each
        returned ``EvalResult`` uses the matching config for summaries and saving.
        """
        if not configs:
            raise ValueError('configs must be non-empty')
        h0 = reduction_identity_hash(configs[0])
        for c in configs[1:]:
            if reduction_identity_hash(c) != h0:
                raise ValueError(
                    'run_multi_classifier: configs differ beyond classifier; '
                    'cannot share reduction'
                )

        cfg0 = configs[0]
        eval_cfg = cfg0.get('eval', {})
        ...
        splitter = RepeatedHoldout(
            ...
            test_ratio=eval_cfg.get('test_ratio', 0.2),
            stratified=eval_cfg.get('stratified', True),
            random_state=cfg0.get('seed', 42),
        )

        dataset_warning = self._detect_sparse_warning(y, n_classes)
        note = cfg0.get('dataset_note')
        if note:
            dataset_warning = note

        split_metrics_per: list[list] = [[] for _ in configs]
        ...
        for i, (train_idx, test_idx) in enumerate(splitter.split(X, y)):
            ...
            for j, cfg in enumerate(configs):
                ...
                log.info("  [%s] overall=%.4f  balanced=%.4f",
                         ...
                         m['overall_acc'], m['balanced_acc'])

        results = []
        for cfg, split_metrics in zip(configs, split_metrics_per):
            summary = aggregate_split_metrics(split_metrics, dataset_warning)
            summary['dataset'] = dataset.name
            summary['method'] = cfg.get('method_tag', cfg.get('model', {}).get('name', ''))
            summary['classifier'] = cfg.get('classifier', {}).get('name', '')
