...
def _collect_metric_keys(rows):
    ...
    if not rows:
        return []
    available = set(rows[0].keys()) - {'dataset', 'disambig', 'elapsed_s', 'error'}
...
def print_summary_table(rows):
    ...
    hdr_fmt = "{:<20s} {:<12s}" + " {:>12s}" * len(keys)
    row_fmt = "{:<20s} {:<12s}" + " {:>12s}" * len(keys)
    print(hdr_fmt.format("Dataset", "Disambig", *keys))
    print("-" * (34 + 13 * len(keys)))
    ...
    for r in rows:
        ...
        print(row_fmt.format(r['dataset'], r['disambig'], *vals))
...
def save_summary_csv(rows, path):
    ...
    fieldnames = ['dataset', 'disambig'] + keys + ['elapsed_s']
...
def save_summary_latex(rows, path):
    ...
    lines = [
        ...
        r"\toprule",
        "Dataset & Disambig & " + " & ".join(header_labels) + r" \\",
        r"\midrule",
    ]
    ...
    for r in rows:
        ...
        if 'error' in r:
            ...
            vals = [f"{r[k]:.4f}" for k in keys]
        lines.append(f"{ds_col} & {r['disambig']} & " + " & ".join(vals) + r" \\")
